...
$(function() {
	...
	// 显示已登录用户nd
	$('#user_info').text('<small>Welcome,</small>'+$.cookie('cuserName'));
...
function getAppUuid(uuid){
    var str=uuid.repleceAll("/","");
	alert(str);
	 return str;
}
...
function resetPasswdReq(token,uuid){
	...
	if(resetPasswdReqFormValidate()){
		$.ajax({
			...
			success:function(respData){
				alert('提示!\n\重置密码成功!');
				window.location.href = '/console/https://console.easemob.com';
			},
			error:function(data){
				alert('提示!\n\重置密码失败!');
			}
		...
		$.ajax({
			...
			success: function(respData, textStatus, jqXHR) {
				...
				$.each(appData,function(key,value){
                    ...
                    //onclick="getAppName(value,key);"
					option += '<tr><td class="text-center"><a href="/console/app_profile/index.html?appUuid='+value+'&Application='+key+'">'+key+'</a></td>'+
...
/*
function getAppName(value,key){
	alert(value+"---"+key);
          var date = new Date();
		  date.setTime(date.getTime()+(1 * 24 * 60 * 60 * 1000));
		  $.cookie('appUuid',value,{path:'/',expires:date});
          $.cookie('Application',key,{path:'/',expires:date});
}
*/
// 删除某个app
...
function sendUserMessage(){
	...
	if ( messageContent ==''){
		...
		var d = {
		  ...
		  "msg" : {
			  "type" : "txt",
			  "msg" : messageContent //消息内容，参考[聊天记录](http://developer.easemob.com/docs/emchat/rest/chatmessage)里的bodies内容
			  }
		 }
...
function togroupaddAppAdminuserusers(appUuid,groupid){
	window.location.href = '/console/app_qunzu_list/index.html?appUuid=' + appUuid + '&groupid=' + groupid;
}
...
function toAppIMList(owner_username){
	window.location.href = '/console/app_IM_list/index.html?appUuid='+appUuid+'&owner_username='+owner_username;
}
//获取用户好友列表
...
function deleteUserAdmin(appUuid,user_name){
	...
	var str=url.substring(url.lastIndexOf('app_u'),url.lastIndexOf(''));
